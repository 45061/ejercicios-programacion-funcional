...
// const ferraris = cars.filter(item => item.brand === 'Ferrari')
// console.log(ferraris)

const ferraris = (arrCars) =>{
    return arrCars.filter(item => item.brand === 'Ferrari')
}
console.log(ferraris(cars))

// //returns all the cars with models between 1980 and 1990

// const nineties = cars.filter(item => item.model >=1980 && item.model<=1990)
// console.log(nineties)
const ninities = (arrCars) =>{
    return arrCars.filter(item=> item.model>=1980 && item.model<=1990)
}
console.log(ninities(cars))
// //returns an array of strings with the same number of cars. Each element should contain the string “This from costs $” (e.g. for the first element it should say “This Mazda from 1989 costs $5241”).
// const list = cars.map(item=> `This ${item.brand} from ${item.model} costs $${item.price}`)
// console.log(list)

const list = (arrCar)=>{
    return arrCar.map(item => `This ${item.brand} from ${item.model} cost $${item.price}`)
}
console.log(list(cars))

// returns the sum of all prices of the BMW cars

const bmwSum = cars.filter(item=> item.brand === 'BMW')
